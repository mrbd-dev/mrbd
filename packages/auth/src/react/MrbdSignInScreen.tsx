  ...
  return (
    <div className={className} style={{ ...styles.container, ...style }}>
      ...
      {(phase === "starting" || phase === "sending_otp") && (
        <p style={styles.hint}>Preparing a secure sign-in…</p>
      )}
      ...
      {phase === "await_email" && request && (
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <p style={styles.hint}>and enter this code:</p>
          <p style={styles.code}>{formatUserCode(request.userCode)}</p>
          <p style={styles.subtle}>Then type your email on your phone. The rest happens here.</p>
        </div>
      )}

      {phase === "otp" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <p style={styles.hint}>Enter the {otpLength}-digit code we emailed you.</p>
